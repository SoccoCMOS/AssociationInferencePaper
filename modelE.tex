...
\mynote[author=EG]{Are the indices on $a$ the right way around?}
\begin{equation*}
...
\mynote[author=EG]{Explain more, which distribution for which scenario, make the columns of the table match the text. What does $\sigma$ stand for?}
...
Our model is then trained on this collection of labelled examples. Specifically, we maximize the log-likelihood \mynote[author=EG]{you minimize the negative log-likelihood, hence maximized the likelihood, right?} of the observed abundances on each site for all positive examples. \mynote[author=EG]{what about negative examples?}
...
\right.
\end{equation*}
\mynote[author=EG]{could also be a small threshold $\epsilon$, rather than 0?}
...
\mynote[author=EG]{this need more explanations, here or where this is discussed on the real-world example}
...
\mynote[author=EG]{I am not sure what it meant when a field was absent in the simulations configurations (except maybe dense/sparse for 5 species, as it is so small)}

\mynote[author=EG]{the connection between the paragraphs above and below are not clear to me. Clarify what are the parameters given to the simulation (matrix of interactions, ...), and highlight them below. Did you use random or hand-crafted initial compositions?}
...
\mynote[author=EG]{how do you measure deviance (is it really deviance or deviation?)}
...
\mynote[author=EG]{That does not seem to appear anywhere in the results plots}
...
\mynote[author=EG]{what is the (average) distribution of negative/positive/neutral in the ground truth for the different cases?}
...
\mynote[author=EG]{This section is difficult to understand. As far as I get it, the point of the two measures below is to capture how clearly the association types specified by the simulation parameters actually appear in the simulated data. Where the pattern are a bit obfuscated, it explains why the model might not be able to recover the association as expected. So the results from the two measures are not an evaluation of the prediction but help explain it, when put next to the metric in the paragraph above, right?}
...
\end{equation*}
Values of $J_{s,t}$ close to $1$ indicate high overlap whereas values close to $0$ indicate near-separation. Values around $0.5$ could indicate average overlap, no conclusion can be drawn. \mynote[author=EG]{I am not sure what average overlap means. The expected overlap also depends on the total number of sites} 
...
\mynote[author=EG]{does that mean you evaluate the prediction performance of the entire model, or only the HSM block?}
...
\mynote[author=EG]{I think it is somewhat missleading to present them stacked in this way because the information is not necessarily additive like this,  some of the same information about the traits might be encoded in different dimensions of the embedding, if I understand correctly}
...
\mynote[author=EG]{this needs more explanations}
